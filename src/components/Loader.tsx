...
const Loader: React.FC<LoaderProps> = ({ fullScreen = false }) => {
  return (
    <StyledWrapper $fullScreen={fullScreen}>
      ...
      <BackgroundAnimation>
        {[...Array(20)].map((_, i) => (
          <FloatingShape key={i} $delay={i * 0.2} $duration={3 + (i % 3)} $left={Math.random() * 100} />
        ))}
        {[...Array(15)].map((_, i) => (
          <FloatingCircle key={`circle-${i}`} $delay={i * 0.15} $duration={4 + (i % 2)} $left={Math.random() * 100} />
        ))}
      </BackgroundAnimation>
        ...
        <svg id="svg-global" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 94 136" height={136} width={94}>
          ...
          <g id="node-server">
            ...
            <g mask="url(#mask0_204_217)">
              <path fill="#2F4F2F" d="M78.3486 68.7324L49.0242 51.0584C47.6459 50.2276 45.4111 50.2276 44.0328 51.0584L14.7084 68.7324C13.3301 69.5631 13.3301 70.91 14.7084 71.7407L44.0328 89.4148C45.4111 90.2455 47.6459 90.2455 49.0242 89.4148L78.3486 71.7407C79.7269 70.91 79.727 69.5631 78.3486 68.7324Z" />
              ...
              <g mask="url(#mask1_204_217)">
                <path fill="#556B2F" d="M68.9898 68.7324L49.0242 56.699C47.6459 55.8683 45.4111 55.8683 44.0328 56.699L24.0673 68.7324C22.6889 69.5631 22.6889 70.91 24.0673 71.7407L44.0328 83.7741C45.4111 84.6048 47.6459 84.6048 49.0242 83.7741L68.9898 71.7407C70.3681 70.91 70.3681 69.5631 68.9898 68.7324Z" />
                <path fill="#6B8E23" d="M70.1311 69.3884L48.42 56.303C47.3863 55.6799 45.7103 55.6799 44.6765 56.303L22.5275 69.6523C21.4938 70.2754 21.4938 71.2855 22.5275 71.9086L44.2386 84.994C45.2723 85.617 46.9484 85.617 47.9821 84.994L70.1311 71.6446C71.1648 71.0216 71.1648 70.0114 70.1311 69.3884Z" />
                <path fill="#8B6F47" d="M70.131 70.8923L48.4199 57.8069C47.3862 57.1839 45.7101 57.1839 44.6764 57.8069L22.5274 71.1562C21.4937 71.7793 21.4937 72.7894 22.5274 73.4125L44.2385 86.4979C45.2722 87.1209 46.9482 87.1209 47.982 86.4979L70.131 73.1486C71.1647 72.5255 71.1647 71.5153 70.131 70.8923Z" />
                <path fill="#A0826D" d="M69.751 72.1675L48.4199 59.3111C47.3862 58.6881 45.7101 58.6881 44.6764 59.3111L23.2004 72.2548C22.1667 72.8779 22.1667 73.888 23.2004 74.5111L44.5315 87.3674C45.5653 87.9905 47.2413 87.9905 48.2751 87.3674L69.751 74.4238C70.7847 73.8007 70.7847 72.7905 69.751 72.1675Z" />
                <path fill="#B8A082" d="M68.5091 72.9231L48.4199 60.8153C47.3862 60.1922 45.7101 60.1922 44.6764 60.8153L24.8146 72.7861C23.7808 73.4091 23.7808 74.4193 24.8146 75.0424L44.9038 87.1502C45.9375 87.7733 47.6135 87.7733 48.6473 87.1502L68.5091 75.1794C69.5428 74.5563 69.5428 73.5462 68.5091 72.9231Z" />
                <path fill="#C4B5A0" d="M66.6747 73.3219L48.4199 62.3197C47.3862 61.6966 45.7101 61.6966 44.6764 62.3197L26.4412 73.3101C25.4075 73.9332 25.4075 74.9433 26.4412 75.5664L44.696 86.5686C45.7297 87.1917 47.4058 87.1917 48.4395 86.5686L66.6747 75.5782C67.7084 74.9551 67.7084 73.945 66.6747 73.3219Z" />
              </g>
...
const StyledWrapper = styled.div<{ $fullScreen?: boolean }>`
  ...
  ${props => props.$fullScreen ? 'min-height: 100vh;' : 'min-height: auto;'}
  width: 100%;
  background: linear-gradient(135deg, #1C3A1C 0%, #2F4F2F 25%, #3D5A3D 50%, #556B2F 75%, #6B8E23 100%);
  overflow: hidden;
  z-index: 0;
`;
...
const FloatingShape = styled.div<{ $delay: number; $duration: number; $left: number }>`
  position: absolute;
  left: ${props => props.$left}%;
  bottom: -50px;
  width: ${() => 20 + Math.random() * 40}px;
  height: ${() => 20 + Math.random() * 40}px;
  background: linear-gradient(135deg, rgba(139, 111, 71, 0.3), rgba(107, 142, 35, 0.2));
...
const FloatingCircle = styled.div<{ $delay: number; $duration: number; $left: number }>`
  position: absolute;
  left: ${props => props.$left}%;
  bottom: -30px;
  width: ${() => 15 + Math.random() * 25}px;
  height: ${() => 15 + Math.random() * 25}px;
  background: radial-gradient(circle, rgba(160, 130, 109, 0.4), rgba(107, 142, 35, 0.2));
  border-radius: 50%;
  animation: ${floatCircle} ${props => props.$duration}s linear infinite;
  animation-delay: ${props => props.$delay}s;
