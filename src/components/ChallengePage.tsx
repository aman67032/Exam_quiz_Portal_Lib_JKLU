...
const ChallengePage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchChallenge = async () => {
            const SAMPLE_CHALLENGE: DailyChallenge = {
                ...
                explanation: '## Approach\n\nIn Python, strings can be sliced using the syntax `[start:stop:step]`.\n\nBy setting the `step` to `-1`, we can generate a copy of the string in reverse order.\n\n### Complexity\n- **Time Complexity:** O(n), where n is the length of the string.\n- **Space Complexity:** O(n) to store the new string.',
                media_link: 'https://images.unsplash.com/photo-1542831371-29b0f74f9713?q=80&w=2070&auto=format&fit=crop',
            ...
            try {
                ...
                try {
                    ...
                    console.warn('Using sample challenge data');
                    setChallenge(SAMPLE_CHALLENGE);
                }
            } catch (err) {
                console.error(err);
                setChallenge(SAMPLE_CHALLENGE);
